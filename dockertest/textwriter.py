"""
Adapted TextWriter from Sphinx implementation for use by documentation module.

How it works: In a nuttshell, every node in the DocTree tree causes
a call to a "visit" and "depart" method depending on the node type.
Each method then formats the node's data for presentation via a Docutils
instance publish methods.  Ref: http://en.wikipedia.org/wiki/Visitor_pattern

Changes from Sphinx version:  Removed required ``Builder`` parameter/class
since only a single string-stream is used for input and output.  Removed
some module-level globals/classes and replaced them inline where they
where used.  Many small formatting changes so the code passes most important
PEP8 checks.
"""
...
class TextWrapper(textwrap.TextWrapper):
    ...
    @staticmethod
    def _break_word(word, space_left):
...
# Visitor class interface was designed with a TON of attributes/methods
# most are documented in the docutils source, no need to re-document here.
# pylint: disable=R0902,C0111
class TextTranslator(nodes.NodeVisitor):
    sectionchars = '*=-~"+`'

    # NodeVisitor interface requires many methods, most end up with
    # unused arguments, but are left here for further output refinement.
    ...
    def __init__(self, document):
        ...
        self.body = []
        self._citlabel = ''
        self._title_char = self.sectionchars[0]
    ...
    @staticmethod
    def visit_tabular_col_spec(node):
        raise nodes.SkipNode
    ...
    # Originally written with large branches
    ...
    @staticmethod
    def visit_toctree(node):
        raise nodes.SkipNode

    @staticmethod
    def visit_substitution_definition(node):
        raise nodes.SkipNode
